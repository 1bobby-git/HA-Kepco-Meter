...
- 버전: `v0.3.10`. 선택 고객마다 5개 논리 기기와 34개 센서 엔티티를 생성합니다.
...
- 세션 만료/복원 실패: `v0.3.10`부터 refresh token과 허용된 `JSESSIONID`/`kepcoSSO` 인증 쿠키를 함께 복원하고, 성공한 보호 요청에서 갱신된 쿠키를 다시 저장합니다. 한전 서버가 세션 자체를 만료한 경우에는 재인증이 필요하며, 비밀번호 저장을 켠 항목은 자동 재로그인을 시도합니다.
