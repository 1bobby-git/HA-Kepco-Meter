...
- 버전: 현재 소스와 최신 공개 GitHub 릴리스는 `v0.1.1`입니다. 릴리스 ZIP, HACS/Hassfest 검증, HAOS 정확 릴리스 배포까지 확인했습니다.
...
릴리스 태그는 매니페스트 `version`, Git 태그, GitHub 릴리스 제목을 일치시킨 뒤 생성합니다. 현재 공개 릴리스와 이 브랜치의 소스 메타데이터는 모두 `v0.1.1`입니다. 릴리스 자산은 `kepco_on-v0.1.1.zip`이며, 정확한 태그 커밋의 Tests, HACS, Hassfest가 모두 통과했습니다.
