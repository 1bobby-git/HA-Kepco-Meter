...
from custom_components.kepco_on.exceptions import (
    KepcoOnAuthError, KepcoOnConnectionError, KepcoOnProtocolError,
    KepcoOnRateLimitError, KepcoOnSessionExpired,
)
        ...
        *, submission_id: str | None = None,
        ...
        if path == ENDPOINT_APT_BILL_DETAIL:
            return cast("dict[str, object]", json.loads(
                (Path(__file__).parent / "fixtures/bill_latest.json").read_text(encoding="utf-8")
            ))
...
@pytest.mark.parametrize("field", FIELDS)
@pytest.mark.parametrize("value", [True, False, -1, "NaN", "Infinity", "-Infinity", "bad", [], {}, 10**400])
...
async def test_real_parsers_publish_both_values_to_ha_and_template(hass: HomeAssistant, code: str | None) -> None:
    bill, auth = await fetch({"dma_powerPlanner": {"RETURN_CD": code, "F_AP_QT": "246800", "PREDICT_TOT": "987654"}})
    item = customer()
    sensors = await publish(hass, await setup_entities(customers=(item,), bills_by_customer_key={item.stable_key: bill}))
    ...
    assert planner_search == {
        "schYm": "", "custNo": "TEST_BUILDING", "gubun": "", "schChart": "12",
        "CUST_NO": "", "housCntrNo": "TEST_HOUSEHOLD", "yyyymm": "", "searchType": "",
        "dong": "", "ho": "", "months": "", "chgYmd": "",
    }
...
@pytest.mark.parametrize(("bad", "status"), [(None, "no_data"), ("NaN", "invalid_response"), (True, "invalid_response")])
async def test_bad_field_never_discards_other_valid_value(hass: HomeAssistant, broken_field: str, bad: object, status: str) -> None:
...
async def test_zero_missing_then_recovery_uses_latest_snapshot(hass: HomeAssistant) -> None:
    ...
    item = customer()
    entities = await setup_entities(customers=(item,), bills_by_customer_key={item.stable_key: bill})
    ...
    for response, expected in scenarios:
        ...
        coordinator.data = replace(coordinator.data, bills_by_customer_key={item.stable_key: next_bill})
...
@pytest.mark.parametrize(("error", "status"), [(KepcoOnConnectionError("PRIVATE_CANARY"), "connection_error"), (KepcoOnRateLimitError("PRIVATE_CANARY"), "rate_limited"), (KepcoOnProtocolError("PRIVATE_CANARY"), "invalid_response")])
async def test_optional_transport_failure_keeps_bill_and_safe_status(error: Exception, status: str) -> None:
    bill, _ = await fetch(error)
    item = customer()
    sensors = by_key(await setup_entities(customers=(item,), bills_by_customer_key={item.stable_key: bill}))
...
@pytest.mark.parametrize("error", [KepcoOnAuthError("expired"), KepcoOnSessionExpired("expired"), asyncio.CancelledError()])
