...
async def test_planner_attributes_are_published_to_states_and_template(
    ...
    item = customer()
    bill = replace(
        synthetic_bill(), current_period_usage_kwh=current,
        power_planner_status=status, power_planner_return_code=code,
    )
    sensors = await publish(hass, await setup_entities(
        customers=(item,), bills_by_customer_key={item.stable_key: bill},
    ))
    for key, field in (("current_period_usage", "F_AP_QT"), ("predicted_period_usage", "PREDICT_TOT")):
        state = hass.states.get(sensors[key].entity_id)
        assert state is not None
        expected = "source_unit_unverified" if key == "predicted_period_usage" and (
            status == "ok" or (status == "no_data" and code == "00")
        ) else status
...
async def test_real_billing_failure_still_marks_all_sensors_unavailable(
    ...
    entities = await setup_entities(
        customers=(item,),
        bills_by_customer_key={} if failure == "missing_bill" else {item.stable_key: synthetic_bill()},
...
async def test_diagnostics_survive_value_loss_and_return_after_billing_recovery(
    ...
    sensors = await publish(hass, await setup_entities(
        customers=(item,), bills_by_customer_key={item.stable_key: initial},
    ))
    coordinator = sensors["current_period_usage"].coordinator
    coordinator.data.bills_by_customer_key[item.stable_key] = replace(
