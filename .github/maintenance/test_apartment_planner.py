...
@pytest.mark.asyncio
@pytest.mark.parametrize("contract", CONTRACTS)
async def test_latest_apartment_planner_uses_household_id_and_full_change_date(contract: str) -> None:
    auth = PlannerAuth({"dma_powerPlanner": {"RETURN_CD": "00", "F_AP_QT": "123.45", "PREDICT_TOT": "99999"}})
    base = synthetic_bill()
    ...
    assert auth.calls == [
        (
            ...
            {"dma_search": {"custNo": "TEST_BUILDING", "housCntrNo": "TEST_HOUSEHOLD", "yymm": "", "yyyymm": "", "searchType": "DETAIL"}},
            "mf_wfm_layout_sbm_search",
        ),
        (
            ENDPOINT_POWER_PLANNER,
            {"dma_search": {"schYm": "", "custNo": "TEST_HOUSEHOLD", "gubun": "", "schChart": "12", "CUST_NO": "", "housCntrNo": "", "yyyymm": "", "searchType": "", "dong": "", "ho": "", "months": "", "chgYmd": "20260409"}},
...
async def test_missing_current_usage_is_not_fabricated(value: object) -> None:
    result = await fetch_latest({"dma_powerPlanner": {"RETURN_CD": "00", "F_AP_QT": value, "PREDICT_TOT": "99999"}})
    assert result.current_period_usage_kwh is None
    assert result.predicted_period_usage_kwh is None
...
async def test_unsuccessful_return_code_never_publishes_numbers(code: str) -> None:
    result = await fetch_latest({"dma_powerPlanner": {"RETURN_CD": code, "F_AP_QT": "123", "PREDICT_TOT": "99999"}})
    assert result.current_period_usage_kwh is None
    assert result.predicted_period_usage_kwh is None
    assert result.power_planner_status == "no_data"
...
async def test_optional_failures_preserve_bills_and_safe_diagnostic_status(error: Exception, status: str) -> None:
...
def test_prediction_unit_is_not_inferred_from_sample_magnitude() -> None:
    for ambiguous in ("636.263", "99999", "garbled", None):
        assert parse_power_planner({"dma_powerPlanner": {"RETURN_CD": "00", "F_AP_QT": "7.5", "PREDICT_TOT": ambiguous}}) == (7.5, None)


@pytest.mark.asyncio
async def test_current_sensor_available_and_prediction_explains_unverified_unit() -> None:
    item = customer()
    current = replace(synthetic_bill(), current_period_usage_kwh=0.0, power_planner_status="ok", power_planner_return_code="00")
    entities = await setup_entities(customers=(item,), bills_by_customer_key={item.stable_key: current})
    ...
    assert sensors["predicted_period_usage"].extra_state_attributes["data_status"] == "source_unit_unverified"


@pytest.mark.asyncio
@pytest.mark.parametrize("status", ["no_data", "not_requested", "connection_error", "rate_limited", "invalid_response"])
